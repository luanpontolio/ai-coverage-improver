...
**Before (Traditional Services)**:
```
apps/backend/src/application/
├── coverage.service.ts         # ❌ Deleted
├── repos.service.ts            # ❌ Deleted
└── improvement.queue.ts        # ❌ Moved to infrastructure
```

**After (Operations Pattern)**:
```
apps/backend/src/
├── domain/                     # ✅ New
│   ├── repository.ts
...
└── infrastructure/
    ...
    │   ├── auth.adapter.ts
    │   └── coverage-source.adapter.ts
    ├── db/                     # ✅ Enhanced with Prisma
    │   ├── prisma.service.ts
    │   ├── coverage.repository.ts
    │   ├── job.repository.ts
    │   └── installation.repository.ts
    └── queue/                  # ✅ Moved from application
        └── index.ts
```

### Controllers Updated

All controllers now inject and execute operations:

- `HealthController` → Health check endpoint
- `AuthController` → `StartGithubAuthOperation`, `CompleteGithubAuthOperation`
- `ReposController` → `ListInstallationRepositoriesOperation`
- `CoverageController` → `AnalyzeRepositoryCoverageOperation`
- `ImprovementsController` → `RequestCoverageImprovementOperation`

### Worker Updated

`improvement.worker.ts` now executes `RunCoverageImprovementOperation` instead of containing business logic.

### Module Registration

`root.module.ts` now registers:
...
1. **Create domain entities first** (`domain/`)
   - Define business rules and validation
   - Keep it framework-independent

2. **Create infrastructure adapters** (`infrastructure/`)
   - If you need to call external APIs
   - If you need new repository methods

3. **Create the operation** (`application/operations/`)
   - Name it after the business interaction
   - Define clear input/output types
   - Orchestrate domain + infrastructure

4. **Create/update controller** (`api/http/controllers/`)
   - Inject the operation
   - Parse HTTP input
   - Call operation
   - Return output

5. **Register in module** (`root.module.ts`)
   - Add operation to providers
   - Add any new adapters/repositories

### Naming Conventions

- **Operations**: `[Verb][Noun]Operation.ts`
  - Examples: `CreateUserOperation`, `SendEmailOperation`
  - Describes the complete interaction

- **Domain Entities**: `[Noun].ts`
  - Examples: `User`, `Order`, `Invoice`
  - Singular, represents a business concept

- **Adapters**: `[System][Purpose].adapter.ts`
  - Examples: `GitHubReposAdapter`, `OpenAIGenerationAdapter`
  - Clearly indicates external system and purpose

---

## Common Questions

### Q: When should I create a new operation vs extend an existing one?

**A**: Create a new operation when you have a distinct business interaction. If you're tempted to add optional parameters to change behavior significantly, you probably need a new operation.

### Q: Can operations call other operations?

**A**: No. This creates hidden workflows and defeats the purpose of explicit interactions. If you need to compose operations, do it in the controller/worker, or create a new operation that orchestrates at the domain level.

### Q: Where does validation happen?

**A**: Input validation happens in operations. Business rule validation happens in domain entities. HTTP-specific validation (like checking content-type) stays in controllers.

### Q: Can I use services at all?

**A**: You can create domain services (framework-independent) if needed for complex business logic. But avoid application-layer "services" that just wrap operations.

### Q: What about shared utility functions?

**A**: Put pure utilities in `infrastructure/` or `shared/`. If they contain business logic, consider if they belong in a domain entity.
